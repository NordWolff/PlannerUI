...
async function onMouseUp() {
  ...
  // Klick ohne nennenswerte Bewegung → Balken dient als Auf-/Zuklapp-Button
...
<template>
  <div class="flex flex-col h-full -mx-4 sm:-mx-6 lg:-mx-8 -my-6">
    ...
    <div class="flex flex-1 overflow-hidden relative">
      <div v-if="projectsStore.loading" class="flex-1 py-8 text-center text-gray-400 text-sm">Laden...</div>
      <div v-else-if="!projectsWithDate.length" class="flex-1 py-8 text-center text-gray-400 text-sm">
        Keine Projekte mit Startdatum
      </div>

      <!-- Zeitstrahl (übernimmt zugleich die Funktion der Projektliste) -->
      <div v-else class="flex-1 overflow-x-auto overflow-y-auto relative" ref="svgContainer">
          ...
          <template v-for="project in projectsWithDate" :key="project.id">
            ...
            <div class="h-10 border-b border-gray-100 dark:border-gray-700/50 relative">
              <div
                ...
                title="Klicken: Tickets auf-/zuklappen · Ziehen: Termin verschieben"
                @mousedown="startDrag($event, project)"
              >
                <svg class="w-3 h-3 mr-1 shrink-0 text-white/80 transition-transform pointer-events-none"
