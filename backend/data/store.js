...
export async function seedData() {
  ...
  store.users.push(
    makeUser(miladId,        'Milad',             'milad@planner.dev',             ownerHash,  'user'),
    makeUser(kayId,          'Kay',               'kay@planner.dev',               ownerHash,  'user'),
  ...
  store.planners.push(
    ...
    },
    {
      ...
      isSystemSupport: true,
      members: store.users.filter(u => u.role === 'admin').map(u => ({ userId: u.id, role: 'owner' })),
      ticketPrefix: 'SUP',
      ticketCounter: 1,
      color: '#6366f1',
      createdAt: now,
